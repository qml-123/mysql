...
class StuvolunteerTable
{
	...
	{
		$result = $this->tableGateway->select(array('tuid'=>$data['stuid'],'cid'=>$data['cid']));
	...
	public function drop($data)
	{
		...
		{
			$result = $this->tableGateway->select(array('tuid'=>$data['tuid'],'cid'=>$data['cid']));
			if(!empty($result))
			{
				$this->tableGateway->delete(array('tuid'=>$data['tuid'],'cid'=>$data['cid']));
			}
		}
		return true;
	}
